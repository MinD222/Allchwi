										...
										<!-- 튜터가 아닌 학생만 과제 제출 가능 -->
											<c:choose>
												<c:when test="${tutor_num != stu_num }">
													<button type="submit" class="btn btn-outline-success" name="subBtn" disabled="disabled">제출하기</button>
												</c:when>
												<c:otherwise>
													<button type="submit" class="btn btn-outline-success" name="subBtn">제출하기</button>
												</c:otherwise>
											</c:choose>
										</div>
									</div>
				...
				$(data).each(function(i,sub){
					...
					if(oldsub != sub.sub_num){
						...
						div.append("<div class='qna_list'>" 
								+"<div class='btnG'><button type='button' class='btn btn-outline-danger'>삭제</button>"
								+"<button type='button' class='btn btn-outline-warning'>수정</button></div>"
									 + "<ul>"
										 + "<li>"
							    			 + "<dl>"
							    				 + "<dt>"
